...
@section('title', 'Sub Vendor Management - Admin')
                        ...
                        <form method="GET" class="row g-3">
                            <div class="col-md-5">
                                <input type="text" name="search" class="form-control"
                                    placeholder="Search sub vendors by contact name, particulars or GST No..." value="{{ request('search') }}">
                            </div>
                            <div class="col-md-2">
                            ...
                            @if (request()->hasAny(['search', 'status']))
                                <div class="col-md-2">
                                    <a href="{{ route('sub-vendors.index') }}" class="btn btn-outline-secondary w-100">
                                        Clear
                                    </a>
                                </div>
                            @endif
                                                    ...
                                                    <ul class="dropdown-menu shadow-lg border-0 py-2"
                                                        ...
                                                        <li>
                                                            <a class="dropdown-item py-2"
                                                                href="{{ route('sub-vendors.show', $vendor) }}">
                                                                <i class="fas fa-eye me-2 text-info"></i>View Profile
                                                            </a>
                                                        </li>
                                                        <li>
                                                            <a class="dropdown-item py-2"
                                                                href="{{ route('sub-vendors.edit', $vendor) }}">
                                                        ...
                                                        <li>
                                                            <button class="dropdown-item py-2 text-danger"
                                                                ...
                                                                <i class="fas fa-trash me-2"></i>Delete Sub Vendor
                                                            </button>
                                                        </li>
                                                    </ul>
                                                </div>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="7" class="text-center py-8">
                                                ...
                                                <h5 class="text-muted mb-3">No sub vendors found</h5>
                                                <p class="text-muted mb-4">Try adjusting search/filter or <a
                                                        href="{{ route('sub-vendors.create') }}"
                                                        class="text-decoration-none">create your first Sub Vendor</a></p>
                                                <a href="{{ route('sub-vendors.create') }}" class="btn btn-primary px-4 py-2">
                                                    <i class="fas fa-plus me-2"></i>Create Sub Vendor
                                                </a>
                                            </td>
                                        </tr>
                                    @endforelse
            ...
            <script>
                ...
                async function deleteVendor(vendorId) {
                    const result = await Swal.fire({
                        title: 'Are you sure?',
                        text: 'This Sub Vendor will be permanently deleted!',
                        icon: 'warning',
                    ...
                    try {
                        const response = await fetch(`sub-vendors/${vendorId}`, {
                            method: 'DELETE',
                            headers: {
                        ...
                        if (response.status) {
                            Swal.fire({
                                icon: 'success',
                                title: 'Deleted!',
                                text: 'Sub Vendor has been deleted.',
                                timer: 2000
                            });
                ...
                async function toggleStatus(vendorId, currentStatus) {
                    ...
                    try {
                        const response = await fetch(`sub-vendors/${vendorId}/toggle-status`, {
                            method: 'POST',
                            headers: {
                                'X-CSRF-TOKEN': CSRF_TOKEN,
                                'Accept': 'application/json',
